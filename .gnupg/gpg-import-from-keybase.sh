...
#
# 암호 입력창은 세 번 뜨지만 새로 기억할 암호는 하나뿐이다.
# Keybase 가 내보낼 키 블록을 감쌀 새 암호를 묻고, 확인을 위해 한 번 더 묻고, 이어서 pinentry 가 같은 값을 다시 묻는다.
# GnuPG 는 그 암호로 블록을 열어 동일한 암호로 private-keys-v1.d 에 저장하므로 이후 커밋 서명 때도 같은 값을 쓴다.
# Keybase 계정 암호는 세션이 만료됐을 때만 그보다 먼저 추가로 묻는다.
#
# 대상 키는 GPG_KEY_ID 로 바꿀 수 있고, 기본값은 아래 key_id 에 있다.
#
...
# Keybase 가 내보낸 블록은 --allow-secret-key-import 로 받아야 한다.
...
report "확인 명령"
print -- "echo test | gpg --clearsign | gpg --verify"
print -- "echo test | gpg -e -r $key_id | gpg -d"
